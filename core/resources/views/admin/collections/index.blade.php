                        ...
                        <table class="table table--light style--two">
                            ...
                            @forelse($collections as $collection)
                                <tr>
                                    ...
                                    <img src="{{getImage(imagePath()['collection']['path'].'/'.$collection->image)}}" width="100px"/></td>

                                    <td data-label="@lang('Action')">
                                        <a href="javascript:void(0)" class="icon-btn btn--primary ml-1 updateCity"
                                            data-id="{{$collection->id}}"
                                            data-name="{{$collection->name}}"
                                            data-image="{{getImage(imagePath()['collection']['path'].'/'.$collection->image)}}"
                                        ><i class="las la-pen"></i></a>
                                        <button class="icon-btn btn--danger ml-1 removeBtn" data-id="{{ $collection->id }}" data-toggle="tooltip" data-original-title="@lang('Delete')">
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">@lang('Add Collection')</h5>
                ...
                <form action="{{route('admin.collections.store')}}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            <label for="image" class="form-control-label font-weight-bold">@lang('Image') (500*500)</label>
    ...
    <div id="updateCityModel" class="modal fade" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{route('admin.collections.update')}}" method="POST" enctype="multipart/form-data">
                    @csrf
...
@push('breadcrumb-plugins')
    <a href="javascript:void(0)" class="btn btn-sm btn--primary box--shadow1 text--small addCity" ><i class="fa fa-fw fa-paper-plane"></i>@lang('Add Collection')</a>
@endpush
...
<script>
    ...
    $(document).on('click', '.removeBtn', function(e) {
            ...
            if (confirm('Are you sure you want to delete ?')) {
                ...
                $.post("{{ route('admin.collections.delete') }}", 'id=' + id, function(data) {
                    window.location = "{{ route('admin.collections.index') }}";
                });
            }
        });
</script>
