...
class WithdrawalController extends Controller
{
    ...
    public function withdraw_amounts(Request $request) {
        $data = [];
        $pageTitle = 'Withdraw';
        return view('admin.withdraw.withdraw', compact('pageTitle', 'data'));
    }

    public function withdraw_transactions(Request $request) {
        $data = WithdrawTransaction::paginate(10);
        $pageTitle = 'Withdraw Transactions';
        return view('admin.withdraw.withdraw_transactions', compact('pageTitle', 'data'));
    }

    public function withdraw_transaction_details(Request $request, $id) {
        $id = $request->id;
        $data = WithdrawTransaction::where('id', $id)->first();
        $pageTitle = 'Withdraw Transaction Details';
        return view('admin.withdraw.withdraw_transaction_details', compact('pageTitle', 'data'));
    }
    
    public function store_withdrawal_transaction(Request $request) {
        $transaction = WithdrawTransaction::where('signature',$request->signature)
                        ->where('transaction_hash',$request->transaction_hash)
                        ->first();
        
        if(!$transaction) {
            $rec = new WithdrawTransaction();
            $rec->transaction_hash = $request->transaction_hash;
            $rec->symbol = $request->symbol;
            $rec->signature = $request->signature;
            $rec->from_address = $request->from_address; 
            $rec->to_address = $request->to_address;
            $rec->amount = $request->amount;
            $rec->status = $request->status ? 1: 0;
            $rec->block_no = $request->block_no;
            $rec->save();
        }
