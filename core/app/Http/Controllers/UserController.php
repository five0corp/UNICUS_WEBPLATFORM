...
class UserController extends Controller
{
    ...
    public function home(Request $request)
    {
        ...
        $transactions = ProductBidding::with('product')->where('user_id', $user->id);
        $status='all';
        if(isset($request->status) && $request->status=='A'){
            $transactions=$transactions->where('status',$request->status);
            $status=$request->status;
        }
        $transactions=$transactions->orderBy('id','desc')->paginate(10);
        $transactionCount = ProductBidding::where('user_id', $user->id)->count();
        $product['all'] = Product::where('user_id', $user->id)->count();
        $product['sold'] = Order::whereHas('product', function($q) use ($user){
            $q->where('user_id', $user->id);
        })->where('status', 2)->count();

        return view($this->activeTemplate . 'user.dashboard', compact('pageTitle','status', 'transactions', 'emptyMessage', 'user', 'amount', 'product', 'transactionCount'));
    }
    ...
    public function submitProfile(Request $request)
    {
        ...
        // $name=explode(' ',$request->firstname,2);
